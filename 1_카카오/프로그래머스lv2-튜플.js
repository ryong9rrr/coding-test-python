...
function solution(s) {
  const tuples = s
    .slice(2, -2)
    .split('},{')
    .map((strings) => strings.split(',').map((x) => parseInt(x, 10)))

  const table = {}
  tuples.forEach((tuple) => {
    tuple.forEach((x) => {
      if (!table[x]) table[x] = 0
      table[x]++
    })
  })
...
// 테스트 7 〉	통과 (4.52ms, 36.7MB)
// 테스트 8 〉	통과 (10.31ms, 39.1MB)
// 테스트 9 〉	통과 (9.01ms, 36.9MB)
// 테스트 10 〉	통과 (11.32ms, 39.1MB)
// 테스트 11 〉	통과 (13.75ms, 39MB)
// 테스트 12 〉	통과 (32.79ms, 42.4MB)
// 테스트 13 〉	통과 (32.74ms, 42.6MB)
// 테스트 14 〉	통과 (32.45ms, 42.8MB)
